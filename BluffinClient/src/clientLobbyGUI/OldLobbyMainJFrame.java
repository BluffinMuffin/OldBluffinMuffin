...
import clientAILobbyGUI.OldLobbyAIMainJFrame;
import clientGame.ClientPokerPlayerInfo;
import clientGame.ClientPokerTableInfo;
import clientGame.PokerClient;
import clientGameGUI.OldGameTableJFrame;

public class OldLobbyMainJFrame extends JFrame implements IClosingListener<PokerClient>
{
    ...
    private JButton getJAIsButton()
    {
        if (jAIsButton == null)
        {
            ...
            jAIsButton.addActionListener(new java.awt.event.ActionListener()
            {
                ...
                {
                    final OldLobbyAIMainJFrame oldLobby = new OldLobbyAIMainJFrame();
                    oldLobby.setVisible(true);
                    OldLobbyMainJFrame.this.setDefaultCloseOperation(WindowConstants.HIDE_ON_CLOSE);
                    setVisible(false);
                }
            });
        }
    ...
    public static void main(String[] args)
    {
        SwingUtilities.invokeLater(new Runnable()
        {
            public void run()
            {
                final OldLobbyMainJFrame thisClass = new OldLobbyMainJFrame();
    ...
    {
        final LobbyAddTableJDialog form = new LobbyAddTableJDialog(OldLobbyMainJFrame.this, m_playerName, 1);
    ...
    {
        final OldLobbyConnectJDialog form = new OldLobbyConnectJDialog(OldLobbyMainJFrame.this);
        form.setVisible(true);
        if (form.isOK())
        {
            ...
            if (connect(m_serverAddress, m_serverPort))
            {
                ...
                while (!isOk)
                {
                    final LobbyNameUsedJDialog form2 = new LobbyNameUsedJDialog(OldLobbyMainJFrame.this, m_playerName);
