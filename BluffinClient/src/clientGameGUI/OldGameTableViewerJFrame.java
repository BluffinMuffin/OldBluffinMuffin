...
public class OldGameTableViewerJFrame extends OldGameTableAbstractJFrame
{
    ...
    public static void main(String[] args)
    {
        SwingUtilities.invokeLater(new Runnable()
        {
            public void run()
            {
                final OldGameTableViewerJFrame thisClass = new OldGameTableViewerJFrame();
     ...
     */
    public OldGameTableViewerJFrame()
    {
        super();
        initialize();
    }
    ...
    private void initializePokerObserverForGUI()
    {
        m_pokerObserver.subscribe(new ClientPokerAdapter()
        {
            ...
            public void playerTurnEnded(OldPokerPlayerInfo player, OldTypePlayerAction action, int actionAmount)
            {
                ...
                if (action == OldTypePlayerAction.FOLD)
                {
                    php.setPlayerCards(GameCard.NO_CARD, GameCard.NO_CARD);
                }
                if (player.getBet() > 0)
    ...
    private void initializePokerObserverForConsole()
    {
        m_pokerObserver.subscribe(new ClientPokerAdapter()
        {
            ...
            public void playerTurnEnded(OldPokerPlayerInfo player, OldTypePlayerAction action, int actionAmount)
            {
                final OldPokerPlayerAction ppa = new OldPokerPlayerAction(action, actionAmount);
