...
public class OldLobbyAIMainJFrame extends JFrame implements IClosingListener<PokerClient>
{
    ...
    private final List<OldLobbyAIMainJFrame> m_agents = new ArrayList<OldLobbyAIMainJFrame>(); // @jve:decl-index=0:
    ...
    private JButton getJHumanButton()
    {
        if (jHumanButton == null)
        {
            ...
            jHumanButton.setText("switch to Human");
            jHumanButton.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    final OldLobbyMainJFrame oldLobby = new OldLobbyMainJFrame();
                    oldLobby.setVisible(true);
                    OldLobbyAIMainJFrame.this.setDefaultCloseOperation(WindowConstants.HIDE_ON_CLOSE);
                    setVisible(false);
                }
            });
        }
    ...
    public static void main(String[] args)
    {
        SwingUtilities.invokeLater(new Runnable()
        {
            public void run()
            {
                final OldLobbyAIMainJFrame thisClass = new OldLobbyAIMainJFrame();
        ...
        if (jAddTableButton == null)
        {
            ...
            jAddTableButton.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    final LobbyAddTableJDialog form = new LobbyAddTableJDialog(OldLobbyAIMainJFrame.this, m_playerName, m_AIs.size());
        ...
        if (jConnectButton == null)
        {
            ...
            jConnectButton.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    {
                        // Eh bien on connecte
                        final OldLobbyAIConnectJDialog form = new OldLobbyAIConnectJDialog(OldLobbyAIMainJFrame.this, getAIs());
                        form.setVisible(true);
                        if (form.isOK())
                        {
                            ...
                            if (connect(m_serverAddress, m_serverPort))
                            {
                                ...
                                for (final OldTupleAISummary tuple : getAIs())
                                {
                                    final OldLobbyAIMainJFrame client = new OldLobbyAIMainJFrame();
                                    ...
                                    if (client.connect(getAddress(), getPort()))
                                    {
                                        ...
                                        while (!isOk)
                                        {
                                            final LobbyNameUsedJDialog form2 = new LobbyNameUsedJDialog(OldLobbyAIMainJFrame.this, client.getPlayerName());
