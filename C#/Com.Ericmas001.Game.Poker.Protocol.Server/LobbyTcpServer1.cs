...
{
    public class LobbyTcpServer1 : CommandTcpCommunicator1<LobbyObserver>
    {
        private string m_PlayerName = "?";
        private readonly IServerLobby m_Lobby;
        readonly Dictionary<int, GameTcpServer1> m_Tables = new Dictionary<int, GameTcpServer1>();

        public LobbyTcpServer1(TcpClient client, IServerLobby lobby)
            : base(client)
        {
            m_Lobby = lobby;
        }
        ...
        {
            GameTcpServer1 client;
            var game = m_Lobby.GetGame(e.Command.TableId);
            var table = game.GameTable;

            if (game.Params.Lobby.OptionType == LobbyTypeEnum.Training)
                client = new GameTcpServer1(e.Command.TableId, game, m_PlayerName);
            else
                client = new GameTcpServer1(e.Command.TableId, game, DataManager.Persistance.Get(e.Command.PlayerName));
        ...
        {
            var client = (GameTcpServer1)sender;
